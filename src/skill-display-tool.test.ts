...
describe("skill-display-tool", () => {
  it("registers the render tool with ChatGPT Apps metadata", () => {
    ...
    expect(mainDisplayTool?.config._meta?.ui?.resourceUri).toBe("ui://skill-display/v3/skill-display.html");
    expect(mainDisplayTool?.config._meta?.["openai/outputTemplate"]).toBe("ui://skill-display/v3/skill-display.html");
  ...
  it("registers mutation tools as app-only helpers without UI templates", () => {
    ...
    for (const tool of helperTools) {
      expect(tool.config._meta?.ui?.resourceUri).toBe("ui://skill-display/v3/skill-display.html");
